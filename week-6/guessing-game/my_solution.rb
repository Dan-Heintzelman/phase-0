...
#   I think this code requires us to return symbols because symbols are more efficient for Ruby to
#   work with. Even if we were to return the string "low", this would still have to be converted to
#   the symbol :low internally for Ruby to understand it. So this is esentially saving Ruby one
#   step. Also, since symbols are unique, we can assume throughout the program that the object
#   ID of :low and :high and :correct are always going to be the same, so if we run through values
#   on objects of the guessing game class multiple times, we know that :correct is always :correct.
